...
export default function NewJob() {
  ...
  const validPrefixes = prefixList.filter(p => /^[0-9A-F]+$/.test(p) && !p.startsWith('00') && !p.startsWith('FF') && p.length <= 64);
  ...
  useEffect(() => {
    ...
    const tid = setTimeout(() => {
      api.estimate(validPrefixes, backend).then(setEstimate).catch(() => setEstimate(null));
    }, 300);
