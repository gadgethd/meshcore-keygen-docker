...
impl MetalSearcher {
    ...
    pub fn count_batch(&mut self) -> Result<(u64, u32), MetalError> {
        ...
        let matches =
            u32::from_le_bytes([count_host[0], count_host[1], count_host[2], count_host[3]]);
...
pub fn verify_gpu_keygen() -> Result<(), MetalError> {
    ...
    let scalars_buf = device.new_buffer((count as u64) * 32, MTLResourceOptions::StorageModeShared);
